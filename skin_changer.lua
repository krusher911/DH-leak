-- [[ this is an old leak i found (still works) ]]
...
do
    ...
    Utilities.Tween = function(args)
        ...
        local tween =
            ...
            obj,
            duration and TweenInfo.new(duration, Enum.EasingStyle.Quad, Enum.EasingDirection.Out) or
                info and TweenInfo.new(unpack(info)),
            prop
        )
        tween:Play()
    ...
    repeat
        task.wait()
    until client.Character:FindFirstChild("FULLY_LOADED_CHAR")
    ...
    InventoryChanger.Skins = {
        ...
        --[[['Galaxy'] = {
            color = newColorSequence(Color3.new(0, 0, 1)),
            particle = {
                properties = {
        ...
        ["Toy"] = {
            ...
            color = newColorSequence(
                {
                    newColorSequenceKeypoint(0, Color3.new(0, 1, 0)),
    ...
    InventoryChanger.Functions.AddOwnedSkins = function()
        for _, v in ipairs(entries:GetChildren()) do
            ...
            if
                ext_name and skin_name and InventoryChanger.Skins[skin_name] and InventoryChanger.Skins[skin_name].guns and
                    InventoryChanger.Skins[skin_name].guns["[" .. ext_name .. "]"]
             then
    ...
    InventoryChanger.Functions.Equip = function(gun_name, skin_name)
        ...
        if not skin_name then
            if gun and gun.Name == gun_name then
                ...
                if
                    skin_name and InventoryChanger.Skins[skin_name] and InventoryChanger.Skins[skin_name].guns and
                        InventoryChanger.Skins[skin_name].guns[gun.Name]
                 then
    ...
    InventoryChanger.Functions.Start = function(name)
        for i, v in next, entries:GetChildren() do
            ...
            if
                string.find(v.Name, name, 1, true) and InventoryChanger.Skins[skin_name] and
                    InventoryChanger.Skins[skin_name].guns and
                    InventoryChanger.Skins[skin_name].guns["[" .. name .. "]"] and
                    InventoryChanger.Skins[skin_name].guns["[" .. name .. "]"].location
             then
                local Preview = v:FindFirstChild("Preview")
                ...
                if Preview and Button and skinInfo then
                    ...
                    if Equipped and extracted_name then
                        Equipped.Visible =
                            ...
                            InventoryChanger.Skins[skin_name].guns["[" .. extracted_name .. "]"].equipped or
                            false
                        InventoryChanger.Functions.Equip(
                            "[" .. extracted_name .. "]",
                        ...
                        getgenv().InventoryConnections[v.Name] =
                            new_btn.MouseButton1Click:Connect(
                            function()
                                ...
                                for k, x in ipairs(entries:GetChildren()) do
                                    if x.Name:match(regex) == extracted_name and x ~= v then
                                        ...
                                        for _, l in next, InventoryChanger.Skins do
                                            if
                                                _ ~= skin_name and l["[" .. extracted_name .. "]"] and
                                    ...
                                    if
                                        x ~= v and string.find(x.Name, name, 1, true) and
                                            InventoryChanger.Skins[skin_name] and
                                            InventoryChanger.Skins[skin_name].guns and
                                            InventoryChanger.Skins[skin_name].guns["[" .. name .. "]"] and
                                            InventoryChanger.Skins[skin_name].guns["[" .. name .. "]"].location
        ...
        getgenv().InventoryConnections.ChildAdded =
            ...
            function(child)
                if child:IsA("Tool") and child:FindFirstChild("GunScript") then
                    ...
                    if skin_name then
                        if
                            InventoryChanger.Skins[skin_name].color and
                                InventoryChanger.Skins[skin_name].guns[child.Name].equipped
                         then
                            ...
                            InventoryChanger.Skins[skin_name].guns[child.Name].TracerLoop =
                                Ignored.DescendantAdded:Connect(
                                function(descendant)
                                    ...
                                    if gun and descendant:IsDescendantOf(siren) and descendant:IsA("Beam") then
                                        local pos1 =
                                            (descendant.Attachment0.WorldCFrame.Position.X >
                                            gun.Handle.CFrame.Position.X) and
                                            descendant.Attachment0.WorldCFrame.Position or
                                            gun.Handle.CFrame.Position
                                        local pos2 =
                                            (descendant.Attachment0.WorldCFrame.Position.X <
                                            gun.Handle.CFrame.Position.X) and
                                            descendant.Attachment0.WorldCFrame.Position or
                                        ...
                                        if
                                            math.abs(client.Character.HumanoidRootPart.Velocity.X) < 22 and
                                                (pos1 - pos2).Magnitude < 5 or
                                                (pos1 - pos2).Magnitude < 20
                                         then
                                            local skin_pack = InventoryChanger.Skins[skin_name]
                                            local guns = skin_pack and skin_pack.guns or nil
                                            local tween_duration =
                                                skin_pack and
                                                (skin_pack.tween_duration or
                                                    guns and guns[gun.Name] and guns[gun.Name].tween_duration) or
                                                nil
                                            local width =
                                                skin_pack and
                                                (skin_pack.beam_width or
                                                    guns and guns[gun.Name] and guns[gun.Name].beam_width) or
                                                nil
                                            local color =
                                                skin_pack and
                                                (skin_pack.color or guns and guns[gun.Name] and guns[gun.Name].color) or
                                                nil
                                            local easing_direction =
                                                skin_pack and
                                                (skin_pack.easing_direction or
                                                    guns and guns[gun.Name] and guns[gun.Name].easing_direction) or
                                                nil
                                            local easing_style =
                                                skin_pack and
                                                (skin_pack.easing_style or
                                                    guns and guns[gun.Name] and guns[gun.Name].easing_style) or
                                                nil
    ...
    InventoryChanger.Functions.CharacterAdded(client.Character)
end